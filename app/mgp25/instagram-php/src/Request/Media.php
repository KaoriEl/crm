...
class Media extends RequestCollection
{
    ...
    /**
     * Получаем медиа через графы (что-то типа нового апи инстаграмма)
     * @param $shortcode
     * @return false| object
     */
    public function getMediaByGraphQl($shortcode) {
        $media_info = $this->ig->request('https://www.instagram.com/p/' . $shortcode . '/?__a=1')
            ->addPost('guid', $this->ig->uuid)
            ->addPost('device_id', $this->ig->device_id)
            ->addPost('_csrftoken', $this->ig->client->getToken())
            ->getDecodedResponse(false);
        if(isset($media_info->graphql)) {
            return $media_info->graphql->shortcode_media;
        } else {
            return false;
        }
    ...
    /**
     * Получаем Media ID с помощью запроса графов на сервера инстаграм через шорткод медиа
     * @param $shortcode
     * @return int
     */
    public function getMediaId($shortcode) {
        $media_info = $this->ig->request('https://www.instagram.com/p/' . $shortcode . '/?__a=1')
            ->addPost('guid', $this->ig->uuid)
            ->addPost('device_id', $this->ig->device_id)
            ->addPost('_csrftoken', $this->ig->client->getToken())
            ->getDecodedResponse(false);
        return $media_info->graphql->shortcode_media->id;
    }
